...
class Account < ActiveRecord::Base
  ...
  def calculate_amount
    ...
    self.amount = operations.current.approved(true).income.sum(:amount) - operations.approved(true).expense.current.sum(:amount)
  end
  ...
  def total_amount
    @total_amount ||= operations.approved(true).income.sum(:amount) - operations.approved(true).expense.sum(:amount)
  end

  def debit
    @debit ||= operations.current.approved(true).income.sum(:amount)
  end

  def credit
    @credit ||= operations.current.approved(true).expense.sum(:amount)
  end
  ...
  def editable_by?(usr = nil)
    (usr || User.current).allowed_to?(:edit_accounts, project)
  end

  def destroyable_by?(usr = nil)
    (usr || User.current).allowed_to?(:delete_accounts, project)
  end
  ...
  def check_integrity
    raise Exception.new("Can't delete account") if Operation.where(:account_id => id).any?
  end
