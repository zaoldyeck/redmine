...
class FinanceMailer < Mailer
  ...
  def account_edit(_user = User.current, operation)
  ...
  def operation_comment_added(_user = User.current, comment)
    ...
    mail :to => operation.recipients,
         ...
         :subject => "Re: [#{operation.project.name}] #{l(:label_operation)}: #{operation}"
  end
end
