...
class AccountsController < ApplicationController
  ...
  before_action :find_account_project, :only => [:new, :create]
  ...
  def create
    @account = Account.new
    @account.safe_attributes = params[:account]
    @account.project = @project
    if @account.save
      Attachment.attach_files(@account, (params[:attachments] || (params[:operation] && params[:operation][:uploads])))
