...
class OperationsController < ApplicationController
  ...
  before_action :find_operation_project, :only => [:new, :create]
  before_action :find_optional_project, :only => :index
  before_action :bulk_find_operations, :only => [:bulk_update, :bulk_edit, :bulk_destroy, :context_menu]
  before_action :authorize, :except => [:index, :auto_complete]
  before_action :find_project_by_project_id, :only => :auto_complete
  ...
  def update
    (render_403; return false) if @operation.linked_payment.nil? && !@operation.editable_by?(User.current)
    @operation.safe_attributes = update_attributes
    @operation.operation_date = calculated_date(params) if [update_attributes[:operation_date], params[:operation_time]].all? { |p| p.present? }
    if @operation.save
