...
class Redmine::ApiTest::ProductCategoriesTest < ActiveRecord::VERSION::MAJOR >= 4 ? Redmine::ApiTest::Base : ActionController::IntegrationTest
  ...
  def test_get_product_categories_xml
    ...
    compatible_api_request :get, '/product_categories.xml', {}

    assert_select 'product_categories', :attributes => { :type => 'array',
  ...
  def test_get_product_categories_1_xml
    ...
    compatible_api_request :get, '/product_categories/1.xml', {}

    assert_select 'product_category', :child => { :tag => 'code', :content => '1' }
  end
  ...
  def test_post_product_categories_xml
    ...
    if ActiveRecord::VERSION::MAJOR < 4
      Redmine::ApiTest::Base.should_allow_api_authentication(:post, '/product_categories.xml', parameters, :success_code => :created)
    end

    assert_difference('ProductCategory.count') do
      compatible_api_request :post, '/product_categories.xml', parameters, credentials('admin')
    end
    ...
    assert_select 'product_category', :child => { :tag => 'id', :content => product.id.to_s }
  end
  ...
  def test_put_product_categories_1_xml
    ...
    if ActiveRecord::VERSION::MAJOR < 4
      Redmine::ApiTest::Base.should_allow_api_authentication(:put, '/product_categories/1.xml', parameters, :success_code => :ok)
    end

    assert_no_difference('ProductCategory.count') do
      compatible_api_request :put, '/product_categories/1.xml', parameters, credentials('admin')
    end
