...
class PopularProductsTest < ActiveSupport::TestCase
  fixtures :projects,
           :users,
           :members,
           :member_roles,
  ...
  def test_datasets_with_today_period
    ...
    chart = RedmineProducts::Charts::PopularProducts.new(query)
    assert_equal 1, chart.datasets.length

    data = query.orders.joins(lines: :product).group('products.name').reorder('sum_product_lines_quantity DESC').limit(10).sum('product_lines.quantity')
