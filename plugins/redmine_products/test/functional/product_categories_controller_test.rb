...
class ProductCategoriesControllerTest < ActionController::TestCase
  ...
  def test_create
    assert_difference 'ProductCategory.count' do
      compatible_request :post, :create, :product_category => { :name => 'New category', :parent_id => 1 }
    end
  ...
  def test_create_failure
    compatible_request :post, :create, :product_category => { :name => '' }
    assert_response :success
  end
  ...
  def test_update
    assert_no_difference 'ProductCategory.count' do
      compatible_request :put, :update, :id => 2, :product_category => { :name => 'Testing' }
    end
    assert_redirected_to '/settings/plugin/redmine_products?tab=product_categories'
  ...
  def test_update_failure
    compatible_request :put, :update, :id => 2, :product_category => { :name => '' }
    assert_response :success
  end

  def test_update_not_found
    compatible_request :put, :update, :id => 97, :product_category => { :name => 'Testing' }
    assert_response 404
  end
